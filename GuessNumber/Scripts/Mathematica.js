        ...
        for (var i = 0; i < 4; i++) {
            ...
            if (i === 3) {
                ...
                if (isComputer === false) {
                    ...
                    var ff = evaluateGuess(gnum, computerSecretNum.toString());                    
                    insCell();                    
                    if (isRestart) { }
                    if (ff.dead === 4) {
                    ...
                    else { let state = env.reset(PlayerOne.number); predictUntilGoal(env, agent); }
                    
                    console.log("comguess = " + comguess);
                    store = comguess;
                    var f = evaluateGuess(comguess.toString(), PlayerOne.number);
                    insCell();
...
var filterAIPossibleNumbers = function (feedback, guess) {
    ...
    pNumbers = pNumbers.filter(nm => {
        let evaluation = evaluateGuess(nm.toString(), guess.toString());
...
async function predictUntilGoal(environment, agent) {
    ...
    const { reward, done: isDone, next_state: nextState } = stepResult;

    if (typeof action === "number") {
        comguess = this.possibleNumbers[action];
        console.log("guess = " + guess);
    } else {
        comguess = action;
    }
        ...
        if (isDone) {
            ...
            fourdead = false;
            console.log("Action chosen:", action, "Feedback:", stepResult.feedback, "Reward:", reward);
        }

        // Here you would typically update the agent's memory for training, 
        // but since you're not retraining, we skip this part
    //}
}
